...
public class s04_LotsOfRectangles {

    public static void main(String[] args) {
        StdDraw.setScale(0, 100);
        StdDraw.enableDoubleBuffering(); //Calling this method stops things from being drawn immediately after a draw method is called. This allows you to call many different draw methods without anything being drawn on the screen; when you call StdDraw.show(), everything will be drawn at once.

        double[] allXPos = {20, 40, 60, 80, 10, 20};
        double[] allYPos = {20, 30, 40, 50, 80, 90};
        double[] allXVel = {10, 50, 20, 100, 5, 2.5};
        double width = 20;
        double height = 20;
        ...
        while (true) {
            ...
            /*
            allXPos[0] = allXPos[0] + xVel*timeElapsed;
            allXPos[1] = allXPos[1] + xVel*timeElapsed;
            allXPos[2] = allXPos[2] + xVel*timeElapsed;
            allXPos[3] = allXPos[3] + xVel*timeElapsed;
            ^^This block of code is equivalent to what we are doing with our for-loop
             */

            for(int i=0; i<allXPos.length; i++){
                allXPos[i] = allXPos[i] + allXVel[i]*timeElapsed;
            }

            for(int i=0; i<allXPos.length; i++){
                if(allXPos[i]-width<0){
                    allXVel[i]*=-1;
                }
                if(allXPos[i]+width>100){
                    allXVel[i]*=-1;
                }
            }
            ...
            for(int i=0; i<4; i++){
                StdDraw.filledRectangle(allXPos[i], allYPos[i], width, height);
            }
