...
public class Start {
    ...
    private Player player1 = new Player(Board.PLAYER_1);

    private Player player2 = new Player(Board.PLAYER_2);
        ...
        if (x == -1) {
            try {
                if (killedPiece != null) {
                    ...
                    if(hasCastled) {
                        if(counter%2==1) {
                            if (Board.board[7][5].equals(Board.ROOK + Board.PLAYER_1)) {
                                ((Rook)player1.getPiece(new Point(7, 5))).hasMoved=false;
                                player1.getPiece(new Point(7, 5)).undoMove(Board.EMPTY);
                            }
                            else if (Board.board[7][3].equals(Board.ROOK + Board.PLAYER_1)) {
                                ((Rook)player1.getPiece(new Point(7, 3))).hasMoved=false;
                                player1.getPiece(new Point(7, 3)).undoMove(Board.EMPTY);
                            }
                        }
                        else {
                            if (Board.board[0][5].equals(Board.ROOK + Board.PLAYER_2)) {
                                ((Rook)player2.getPiece(new Point(0, 5))).hasMoved=false;
                                player2.getPiece(new Point(0, 5)).undoMove(Board.EMPTY);
                            }
                            else if (Board.board[0][3].equals(Board.ROOK + Board.PLAYER_2)) {
                                ((Rook)player2.getPiece(new Point(0, 3))).hasMoved=false;
                                player2.getPiece(new Point(0, 3)).undoMove(Board.EMPTY);
                            }
                        }
                    }
                }
    ...
    private boolean isEmpty(int x, int y) {
        return Board.board[x][y].equals(Board.EMPTY);
    }
}
